...
function ENT:Use(activator,caller)
	...
	self.Entity:EmitSound( "items/gift_drop.wav", 100, 100 )
    self.Entity:Remove()
end

function ENT:CheckOD(activator, caller)
local doses = 0
        if (activator:GetNWBool("Djumped") == true) then
            doses = doses+1
        end
        if (activator:GetNWBool("Adrenaline") == true) then
            doses = doses+1
        end
        if (activator:GetNWBool("Regened") == true) then
            doses = doses+1
        end
        if (activator:GetNWBool("Leeched") == true) then
            doses = doses+1
        end

    if (doses>=3) then
    activator:Kill()
    Notify(activator, 1, 3, "You have overdosed!")
    end
end
...
local function SoberUp(pl, x, y, z, ndeath, didntdie)
    ...
    if timer.Exists ("RegenTimer"..pl:Nick()) then
