...
function ENT:DrawInfo()
	...
	if self.Flip then self.FinishedPos = Pos + self:GetAngles():Up() * 50 + Ang:Up() * self.max.x
	...
	cam.Start3D2D(self.FinishedPos, Ang, 0.15)
		...
		draw.RoundedBox(10, TextWidthName * -0.68, 60, math.Clamp(Health,0,MaxHP) / 2, 20, DamageColorScale)
