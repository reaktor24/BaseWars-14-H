...
function ENT:DrawInfo()
	...
	if self.Flip then self.FinishedPos = Pos + self:GetAngles():Up() * 35 + Ang:Up() * self.max.x
	...
	cam.Start3D2D(self.FinishedPos, Ang, 0.15)
		...
		draw.RoundedBox(10, TextWidthName * -0.68, 30, math.Clamp(Health,0,MaxHP), 20, DamageColorScale)
	cam.End3D2D()
end