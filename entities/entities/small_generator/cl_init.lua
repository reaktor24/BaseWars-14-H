...
function ENT:DrawInfo()
	...
	else self.FinishedPos = Pos + self:GetAngles():Up()* 15 + self:GetAngles():Forward() * 5 + Ang:Up() * self.max.y, Ang, 0.11 end

	cam.Start3D2D(self.FinishedPos, Ang, 0.15)
		draw.WordBox(2, 0 - TextWidthOwner * .5, -60, self:GetBuyer(), "DermaLarge", Color(0, 0, 0, 100), Color(255,255,255,255))
		draw.WordBox(2, 0 - TextWidthName * .5, -20, Name, "DermaLarge", Color(0, 0, 0, 100), Color(255,255,255,255))
		draw.RoundedBox(10, TextWidthName * -0.75, 60, math.Clamp(Health,0,MaxHP), 20, DamageColorScale)
