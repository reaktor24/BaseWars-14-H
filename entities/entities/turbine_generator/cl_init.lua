...
function ENT:DrawInfo()
	...
	else self.FinishedPos = Pos + self:GetAngles():Up()* 10 + self:GetAngles():Forward() + Ang:Up() * self.max.y, Ang, 0.11 end
	...
	cam.Start3D2D(self.FinishedPos, Ang, 0.15)
		...
		draw.RoundedBox(10, TextWidthName * -0.70, 60, math.Clamp(Health,0,MaxHP), 20, DamageColorScale)
